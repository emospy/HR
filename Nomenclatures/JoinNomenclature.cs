...
namespace HR
{
	...
	///</remarks>
	public class JoinNomenklature : System.Windows.Forms.Form
	{
		...
		/// </summary>
		public JoinNomenklature(string table, string formName, ArrayList arr, mainForm main, string descriptor)
		{
			try
			{
		...
		private void buttonDelete_Click(object sender, System.EventArgs e)
		{
			try
			{
				if (this.dataGridView1.CurrentRow != null)
				{
					...
					{
						int index = arr.IndexOf(this.dataGridView1.CurrentRow.Cells["level"].Value.ToString());
			...
			try
			{
				...
				if (form.ShowDialog(this) == DialogResult.OK)
				{
					...
					for (int i = 0; i < ray.Count; i++)
					{
						MappingFormData map = (MappingFormData)ray[i];
						int index = 0;
						index = this.arr.IndexOf(this.dataGridView1.CurrentRow.Cells[map.MappingName].Value.ToString());
						if (index >= 0 && i == 0)
						{
							this.arr[index] = map.ColumnText;
						}
						Dict.Add(map.MappingName, map.ColumnText);
		...
		private void JoinNomenklature_Load(object sender, EventArgs e)
		{
			try
			{
				this.dt = this.da.SelectWhere(this.table, "*", "where descriptor = '" + descriptor + "'");
				if (dt == null)
				{
