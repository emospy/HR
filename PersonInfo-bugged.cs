...
namespace LichenSystaw2004
{
	...
	{		
		...
		private void buttonAssignmentSave_Click(object sender, System.EventArgs e)
		{
			...
			if( !IsAssignmentEdit )
			{
				...
				if( !IsUnvalid )
				{
					...
					else //При назначаване се добавя пропорциаонално отпуска ако сме назначени текущата година или се назначава цялата отпуска при старо назначение
					{
						...
						rowH["id"] = this.assignmentAction.GetLastInsertID();
			...
			else	// Towa e pri update
			{
				...
				if( row != null )
				{
					...
					foreach(DataRow r in this.dtYearHoliday.Rows )
					{
						if( r["year"].ToString() == DateTime.Now.Year.ToString() )
						{
							rowH = r;
							break;
						}
					...
					if( rowH != null ) // Prowerka dali e nameren red za tekushtata godina
					{
					...
					else // ako ne e nameren to se wkarwa now red za nowata godina
					{
						MessageBox.Show("Не е намерен запис за отуските за текущата година. Моля проверете дали е извършено годишно приключване за предходната година");
					}
					this.personAction.UpdatePersonPosition(this.parent, this.nodeID);
				}
		...
		{
						formYearAdd form = new formYearAdd( this.parent, "year_holiday", this.dtYearHoliday,
							this.mainform );
						form.ShowDialog();
